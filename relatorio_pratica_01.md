...
- **B:** `mesmaEspecie`
- **C:** `modoInter`
- **D:** `o1.saude > threshold` (Dado o bug, simplificamos a redundância)
- **E:** `o1.invasora`
- **F:** `o2.invasora`

_Tabela de pares de independência (Resumo para Cobertura)_:
Para garantir MC/DC, cada condição deve demonstrar sua capacidade de alterar o resultado da decisão (T e F), mantendo as demais fixas.
...
A ferramenta PIT (PIT Mutation Testing) introduz falhas artificiais (mutantes) no código, como trocar `<` por `<=`, ou `&&` por `||`. O objetivo dos nossos testes é "matar" esses mutantes.
...
Abaixo está a suíte JUnit desenvolvida para cobrir as especificações e a tabela MC/DC.
